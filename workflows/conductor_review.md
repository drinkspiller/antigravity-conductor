...
3. **Load Context:** Read:
   - `{PROJECT_ROOT}/conductor/tracks.md` — Identify the active track (marked
     `[~]` or most recently `[x]`).
   - The active track's `spec.md` and `plan.md`.
   - `{PROJECT_ROOT}/conductor/product-guidelines.md`
   - `{PROJECT_ROOT}/conductor/workflow.md`
   - Any relevant files in `{PROJECT_ROOT}/conductor/code_styleguides/`
4. **Determine Review Scope:** Ask the user: **"What would you like me to
   review? (A) The entire track, (B) Just the current/latest phase, or (C) A
   specific set of files?"**
   Wait for the user's response.
5. **Gather Changed Files:** Using the workspace's VCS (detect whether it is
   git, hg/Mercurial, or jj):
   - For the selected scope, run the appropriate diff command against the
     track start point:
...
6. **Evaluate Against Criteria:**
   ...
   - Mark as ✅ Met, ⚠️ Partially met, or ❌ Not met.
     Check quality gates from `conductor/workflow.md`: - Test coverage threshold
     met? - All tests passing? - Commit messages follow convention?
     Check style guide compliance from `conductor/code_styleguides/`: - Naming
     conventions followed? - File organization correct? - Import ordering
     consistent?
7. **Generate Review Report:**
   Write the review report as a Jetski artifact (save to
   `{PROJECT_ROOT}/conductor/tracks/<track_id>/review.md`, `ArtifactType:
walkthrough`). Use this structure:
   ```
   ...
   ```
8. **Ask for Disposition:** Present the review artifact via `notify_user` with
   `PathsToReview` pointing to the `review.md` file (`BlockedOnUser: true`).
   Include in the `Message`: "Based on this review, would you like to: (A) Mark
   the track as complete, (B) Create follow-up tasks for the issues found, or
   (C) Continue implementation to address the gaps?"
